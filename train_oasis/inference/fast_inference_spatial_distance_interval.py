"""
Oasis inference with interval best-of-N using spatial distance (VGGT + Chamfer).

Same chunk-wise selection schedule as ``fast_inference_droid_interval.py``, but
each interval scores candidates with ``utils.spatial_distance`` instead of Droid.
"""
import sys
import os
import tempfile
...
def write_gt_prefix_temp(gt_path: str, video_offset: int, target_len: int, fps: int) -> str:
    """Write frames ``[video_offset : video_offset + target_len)`` from GT video to a temp mp4."""
    frames, _, _ = read_video(gt_path, pts_unit="sec")
    end = video_offset + target_len
    if end > frames.shape[0]:
        raise ValueError(
            f"GT video too short for prefix: need end={end}, have T={frames.shape[0]} ({gt_path})"
        )
    seg = frames[video_offset:end]
    fd, tmp_path = tempfile.mkstemp(suffix=".mp4")
    os.close(fd)
    write_video(tmp_path, seg.cpu().numpy(), fps=fps)
    return tmp_path
...
def main(args):
    ...
    for start_idx in tqdm(range(0, len(items), args.batch_size), desc="dataset batches"):
        ...
        def generate_interval(x_state, target_len):
            x_gen = x_state.clone()
            for i in range(x_gen.shape[1], target_len):
                chunk = torch.randn((B, 1, *x_gen.shape[-3:]), device=device)
                chunk = torch.clamp(chunk, -noise_abs_max, +noise_abs_max)
                x_gen = torch.cat([x_gen, chunk], dim=1)
                ...
                for noise_idx in reversed(range(1, ddim_one_step + 1)):
                    ...
                    x_gen[:, start_frame + 1 :] = x_pred[:, -(horizon - 1) :]
            return x_gen

        def refine_tail(x_state):
            x_ref = x_state.clone()
            for i in range(total_frames, total_frames + max_frame - 2):
                ...
                for noise_idx in reversed(range(1, ddim_one_step + 1)):
                    ...
                    if noise_idx == 1:
                        alpha_next[:, 1:2] = torch.ones_like(alpha_next[:, 1:2])
                    x_pred = alpha_next.sqrt() * x_start + x_noise * (1 - alpha_next).sqrt()
                    x_ref[:, start_frame + 1 :] = x_pred[:, -(horizon - 1) :]
            return x_ref

        def decode_for_score(x_latent):
            x_out = x_latent
            if args.vae_ckpt:
                t_len = x_out.shape[1]
                ...
                x_out = rearrange(x_out, "(b t) c h w -> b t h w c", t=t_len)
            x_out = x_out * 0.5 + 0.5
            x_out = torch.clamp(x_out, 0, 1)
            return x_out

        x_best = x.clone()
        cur_len = x_best.shape[1]
        interval_idx = 0
        while cur_len < total_frames:
            target_len = min(total_frames, cur_len + args.chunk_frames)
            gt_tmp_paths = []
            gt_prefix_pcs = []
            for b in range(B):
                gt_tmp = None
                try:
                    gt_tmp = write_gt_prefix_temp(
                        gt_video_paths[b], args.video_offset, target_len, args.fps
                    )
                    gt_tmp_paths.append(gt_tmp)
                    gt_prefix_pcs.append(sd_reconstructor.reconstruct_video(gt_tmp))
                except Exception:
                    for pth in gt_tmp_paths:
                        _unlink_quiet(pth)
                    raise
            ...
            for p in range(args.num_particles):
                seed = args.base_seed + p + 1000 * total_frames + 100000 * interval_idx
                torch.manual_seed(seed)
                torch.cuda.manual_seed(seed)

                x_cand = generate_interval(x_best, target_len)
                x_out = decode_for_score(x_cand)

                for b in range(B):
                    gen_tmp = None
                    try:
                        fd, gen_tmp = tempfile.mkstemp(suffix=".mp4")
                        os.close(fd)
                        write_video(
                            gen_tmp,
                            (x_out[b, :target_len] * 255).byte().cpu().numpy(),
                            fps=args.fps,
                        )
                        gen_pc = sd_reconstructor.reconstruct_video(gen_tmp)
                        ...
                        print(
                            f"{os.path.basename(save_paths[b])} interval={interval_idx} "
                            f"len={target_len} particle={p} seed={seed} "
                            f"sd_{args.sd_metric}={raw:.6f} score={cand_scores[p, b].item():.6f}"
                        )
                    finally:
                        _unlink_quiet(gen_tmp)

                cand_latents.append(x_cand.detach())
            ...
            for b in range(B):
                ...
                print(
                    f"{os.path.basename(save_paths[b])} selected interval={interval_idx} "
                    f"particle={best_idx} argmax_score={best_score:.6f}"
                )
                next_best.append(cand_latents[best_idx][b : b + 1])
            x_best = torch.cat(next_best, dim=0)
            cur_len = target_len
            interval_idx += 1

        x_best = refine_tail(x_best)
        x_final_all = decode_for_score(x_best).detach().cpu()
        for b, save_path in enumerate(save_paths):
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
            write_video(save_path, (x_final_all[b] * 255).byte().numpy(), fps=args.fps)
...
if __name__ == "__main__":
    ...
    parse.add_argument("--chunk-frames", type=int, default=10, help="Frames per chunk for BoN selection (frequency).")
