...
def main(args):
    ...
    chunk_scores = []
    for chunk_start in range(n_prompt_frames, total_frames, chunk_size):
        chunk_end = min(chunk_start + chunk_size, total_frames)
        cand_scores = []
        cand_states = []
        for p in range(args.num_particles):
            seed = args.base_seed + p + 1000 * chunk_start
            x_cand = _generate_chunk(x_work.clone(), chunk_start, chunk_end, seed=seed)
            cand_states.append(x_cand)
            full_video = _decode_latents(x_cand)[0]
            score = scorer.reward_from_video(full_video)
            cand_scores.append(score)
            print(f"Chunk {chunk_start}:{chunk_end} seed={seed} score={score}")
        best_idx = int(np.argmax(cand_scores))
        x_work = cand_states[best_idx]
        chunk_scores.append((chunk_start, chunk_end, cand_scores, best_idx))

    # finalize tail refinement + decode + save
    x_work = _refine_tail(x_work)
    x_final = _decode_latents(x_work)[0]
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    write_video(output_path, (x_final * 255).byte().cpu().numpy(), fps=args.fps)
    print(f"generation saved to {output_path}.")
    print("Chunk selection summary:")
    for chunk_start, chunk_end, scores, best_idx in chunk_scores:
        print(f"{chunk_start}:{chunk_end} scores={scores} best={best_idx}")
