...
@torch.no_grad()
def rnn_inference_batch(model_name, ckpt_path):
    from train_oasis.model.rnn_dit import DiT
    ...
    os.makedirs(save_dir, exist_ok=True)
    with open(metadata_file_path, 'r') as f:
        paths = json.load(f)[:200]
    paths = sorted(paths, key=lambda x: x["length"], reverse=True)[50:150]
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    vae_ckpt = "stabilityai/sd-vae-ft-mse"
    ...
    for start_idx in range(0, len(prompts), inference_batch):
        # sampling inputs
        end_idx = min(start_idx + inference_batch, len(prompts))
        video_lengths = [prompts[i].shape[1] for i in range(start_idx, end_idx)]
        max_video_length = max(video_lengths)
        min_video_length = min(video_lengths)
        n_prompt_frames = max(10, int(max_video_length * 0.5))
        action_length = min_video_length
        assert action_length >= n_prompt_frames, f"action_length {action_length} < n_prompt_frames {n_prompt_frames}"
        x = torch.cat([prompts[i][:, :n_prompt_frames] for i in range(start_idx, end_idx)], dim=0)
        actions = torch.cat([all_actions[i][:, :action_length] for i in range(start_idx, end_idx)], dim=0)
        B = x.shape[0]
        H, W = x.shape[-2:]
        # assert B == batch_size, f"{B} != {batch_size}"
        # x = torch.randn((B, 1, *x.shape[-3:]), device=device) # (B, 1, C, H, W)
