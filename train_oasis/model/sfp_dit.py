...
class FinalTemporalAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        q = self.rotary_emb.rotate_queries_or_keys(q, self.rotary_emb.freqs)[:, :, -1:, :]
...
class FinalSpatioTemporalDitBlock(nn.Module):
    def __init__(
        self,
        hidden_size,
        num_heads,
        ...
        self.t_attn = FinalTemporalAttention(
            hidden_size,
            heads=num_heads,
            dim_head=hidden_size // num_heads,
            is_causal=is_causal,
    ...
    def forward(self, x, c):
        ...
        # temporal block
        t_shift_msa, t_scale_msa, t_gate_msa, t_shift_mlp, t_scale_mlp, t_gate_mlp = self.t_adaLN_modulation(c[:, -1:]).chunk(6, dim=-1)
        x = x[:, -1:] + gate(self.t_attn(modulate(self.t_norm1(x), t_shift_msa, t_scale_msa)), t_gate_msa)
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.final_block = FinalSpatioTemporalDitBlock(
            hidden_size,
            num_heads,
            mlp_ratio=mlp_ratio,
            is_causal=True,
    ...
    def initialize_weights(self):
        ...
        # Zero-out adaLN modulation layers in final block:
        nn.init.constant_(self.final_block.s_adaLN_modulation[-1].weight, 0)
        nn.init.constant_(self.final_block.s_adaLN_modulation[-1].bias, 0)
        nn.init.constant_(self.final_block.t_adaLN_modulation[-1].weight, 0)
        nn.init.constant_(self.final_block.t_adaLN_modulation[-1].bias, 0)
    ...
    def forward(self, x, t, external_cond=None):
        ...
        # final block
        if self.gradient_checkpointing and self.training:
            x = checkpoint(self.final_block, x, c, use_reentrant=False)
        else:
