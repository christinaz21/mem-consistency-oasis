...
class TemporalAxialAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, memory: torch.Tensor):
        ...
        x = torch.cat([memory, x], dim=1)  # (B, T+M, H, W, D), M is memory length
        ...
        x = x[:, memory.shape[1]:, :, :, :]  # remove memory tokens
        x = x.type_as(q)
...
class SpatioTemporalDiTBlock(nn.Module):
    ...
    def forward(self, x, c, memory):
        ...
        x = x + gate(self.t_attn(modulate(self.t_norm1(x), t_shift_msa, t_scale_msa), memory), t_gate_msa)
...
class DiT(nn.Module):
    ...
    def forward(self, x, t, memories, external_cond=None):
        """
        Forward pass of DiT.
        x: (B, T, C, H, W) tensor of spatial inputs (images or latent representations of images)
        t: (B, T,) tensor of diffusion timesteps
        memories: (D, B, M, H, W, C) D is layer depth, M is memory length
        """
        ...
        for block, memory in zip(self.blocks, memories):
            if self.gradient_checkpointing and self.training:
                x = checkpoint(block, x, c, memory, use_reentrant=False)
            else:
                x = block(x, c, memory)  # (N, T, H, W, D)
        if self.gradient_checkpointing and self.training:
