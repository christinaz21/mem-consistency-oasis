...
class LatentFramePackVideo(pl.LightningModule):
    def __init__(self, cfg: DictConfig, model_cfg: DictConfig, model_ckpt: str = None):
        ...
        self.clean_frames = cfg.clean_frames
        self.predict_frames = cfg.predict_frames
        assert self.clean_frames + self.predict_frames == self.n_frames, f"clean_frames + predict_frames must equal n_frames, but got {self.clean_frames} + {self.predict_frames} != {self.n_frames}"
        
        self._build_model(model_ckpt)
        self._build_buffer()

    def _build_model(self, model_ckpt):
        if self.model_cfg.architecture == "frame_pack_dit":
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        if nan_number != 0:
            ...
            if self.predict_v:
                target = self.predict_v_from_x(xs[-self.predict_frames:], noise_levels[-self.predict_frames:], noise[-self.predict_frames:])
            else:
                target = xs[-self.predict_frames:]
            model_pred = model_pred[-self.predict_frames:]
            loss = F.mse_loss(model_pred, target.detach(), reduction="none")
            loss_weight = self.compute_loss_weights(noise_levels[-self.predict_frames:])
    ...
    def _generate_noise_levels(self, xs: torch.Tensor, masks: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        clean_noise_levels = torch.randint(0, self.stabilization_level, (self.clean_frames, batch_size), device=xs.device)
        predict_noise_levels = torch.randint(0, self.timesteps, (self.predict_frames, batch_size), device=xs.device)
        noise_levels = torch.cat([clean_noise_levels, predict_noise_levels], dim=0)
        assert noise_levels.shape == (num_frames, batch_size), f"noise_levels shape mismatch: {noise_levels.shape} != ({num_frames}, {batch_size})"
